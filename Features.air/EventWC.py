...
def CheckAutoShowDeal(caseId):
    uId = GetCurUId()
    dealStt = api_getModel(uId, WC_MODEL)["checkedOfferStatus"]
    isOutOfDeal = True
    for packId in range(len(configOffers)):
        if configOffers[packId]["limit"] != dealStt[packId]:
            isOutOfDeal = False
            break
    if isOutOfDeal:
...
#         poco(BTN_CHEAT).click()
    poco(BTN_CHEAT_BR).click()
    dayId = GetCurDayId()
    poco(TXT_PROCESS).click()
    for i in range(2):
        keyevent("KEYCODE_DEL")
    dayId = GetCurDayId()
    if not almostComplete:
        text(str(configChallenge[dayId]["threshold"]))
        poco(BTN_ACCEPT).click()
...
def BuyAllDeal(caseId):
    ...
    for packId in range(len(configOffers)):
        for num in range(configOffers[packId]["limit"]):
            slot = "%s%s" %(NODE_OFFER, packId)
            if dealStt[packId] >= configOffers[packId]["limit"]:
